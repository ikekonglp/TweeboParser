...
public:
  ...
  const vector<int> &GetBrownAllIds() const { return brownall_ids_; }
  ...
  const vector<int> &GetBrown4Ids() const { return brown4_ids_; }
  const vector<int> &GetBrown6Ids() const { return brown6_ids_; }
  ...
  int GetBrownAllId(int i) { return brownall_ids_[i]; };
  ...
  int GetBrown4Id(int i) { return brown4_ids_[i]; };
  int GetBrown6Id(int i) { return brown6_ids_[i]; };
  ...
  int GetHead(int i) { 
    if(i < 0){
      VLOG(2) << "looking for a non-exsiting head";
      return -1;
    }
    return heads_[i]; 
  };
 ...
 private:
  ...
  vector<int> brown4_ids_;
  vector<int> brown6_ids_;
