...
DEFINE_int32(brownall_cutoff, 0,
             "Ignore brown cluster features with all bits whose frequency is less than this.");
...
DEFINE_int32(brown4_cutoff, 0,
             "Ignore brown cluster features with 4 bits whose frequency is less than this.");
DEFINE_int32(brown6_cutoff, 0,
             "Ignore brown cluster features with 4 bits whose frequency is less than this.");
...
void TokenDictionary::Load(FILE* fs) {
  ...
  if (0 > brownall_alphabet_.Load(fs)) CHECK(false);
  ...
  if (0 > brown4_alphabet_.Load(fs)) CHECK(false);
  if (0 > brown6_alphabet_.Load(fs)) CHECK(false);
...
void TokenDictionary::Save(FILE* fs) {
  ...
  if (0 > brownall_alphabet_.Save(fs)) CHECK(false);
  ...
  if (0 > brown4_alphabet_.Save(fs)) CHECK(false);
  if (0 > brown6_alphabet_.Save(fs)) CHECK(false);
  if (0 > shape_alphabet_.Save(fs)) CHECK(false);
}

// LPK: In sequence model, we do not need to read the brown clustering stuff,
// or it should be cheating...
...
void TokenDictionary::InitializeFromReader(DependencyReader *reader) {
  ...
  int form_cutoff = FLAGS_form_cutoff;
  int brownall_cutoff = FLAGS_brownall_cutoff;
  ...
  int brown4_cutoff = FLAGS_brown4_cutoff;
  int brown6_cutoff = FLAGS_brown6_cutoff;
  int prefix_length = FLAGS_prefix_length;
  int suffix_length = FLAGS_suffix_length;
  bool form_case_sensitive = FLAGS_form_case_sensitive;

  vector<int> form_freqs;
  vector<int> brownall_freqs;
  vector<int> lemma_freqs;
  vector<int> feats_freqs;
  vector<int> pos_freqs;
  vector<int> cpos_freqs;
  vector<int> brown4_freqs;
  vector<int> brown6_freqs;  
  Alphabet form_alphabet;
  Alphabet brownall_alphabet;
  Alphabet lemma_alphabet;
  Alphabet feats_alphabet;
  Alphabet pos_alphabet;
  Alphabet cpos_alphabet;
  Alphabet brown4_alphabet;
  Alphabet brown6_alphabet;
  ...
  for (int i = 0; i < NUM_SPECIAL_TOKENS; ++i) {
    ...
    suffix_alphabet_.Insert(special_symbols[i]);
    form_alphabet.Insert(special_symbols[i]);
    brownall_alphabet.Insert(special_symbols[i]);
    ...
    brown4_alphabet.Insert(special_symbols[i]);
    brown6_alphabet.Insert(special_symbols[i]);

    // Counts of special symbols are set to -1:
    form_freqs.push_back(-1);
    brownall_freqs.push_back(-1);
    lemma_freqs.push_back(-1);
    feats_freqs.push_back(-1);
    pos_freqs.push_back(-1);
    cpos_freqs.push_back(-1);
    brown4_freqs.push_back(-1);
    brown6_freqs.push_back(-1);
  }
  ...
  while (instance != NULL) {
    ...
    for (int i = 0; i < instance_length; ++i) {
      ...
      // Add brownall to alphabet.
      string brownall = instance->GetBrownAll(i);
      id = brownall_alphabet.Insert(brownall);
      if (id >= brownall_freqs.size()) {
         CHECK_EQ(id, brownall_freqs.size());
         brownall_freqs.push_back(0);
      }
      ++brownall_freqs[id];
      ...
      // Add brown4 to alphabet.
      string brown4 = instance->GetBrown4(i);
      id = brown4_alphabet.Insert(brown4);
      if (id >= brown4_freqs.size()) {
         CHECK_EQ(id, brown4_freqs.size());
         brown4_freqs.push_back(0);
      }
      ++brown4_freqs[id];


      // Add brown6 to alphabet.
      string brown6 = instance->GetBrown6(i);
      id = brown6_alphabet.Insert(brown6);
      if (id >= brown6_freqs.size()) {
         CHECK_EQ(id, brown6_freqs.size());
         brown6_freqs.push_back(0);
      }
      ++brown6_freqs[id];
  ...
  while (true) {
    brownall_alphabet_.clear();
    for (int i = 0; i < NUM_SPECIAL_TOKENS; ++i) {
    	brownall_alphabet_.Insert(special_symbols[i]);
    }
    for (Alphabet::iterator iter = brownall_alphabet.begin();
         iter != brownall_alphabet.end();
         ++iter) {
      if (brownall_freqs[iter->second] > brownall_cutoff) {
    	  brownall_alphabet_.Insert(iter->first);
      }
    }
    if (brownall_alphabet_.size() < kMaxFormAlphabetSize) break;
    ++brownall_cutoff;
    LOG(INFO) << "Incrementing brownall cutoff to " << brownall_cutoff << "...";
  }
  ...
  while (true) {
    brown4_alphabet_.clear();
    for (int i = 0; i < NUM_SPECIAL_TOKENS; ++i) {
      brown4_alphabet_.Insert(special_symbols[i]);
    }
    for (Alphabet::iterator iter = brown4_alphabet.begin();
         iter != brown4_alphabet.end();
         ++iter) {
      if (brown4_freqs[iter->second] > brown4_cutoff) {
         brown4_alphabet_.Insert(iter->first);
      }
    }
    if (brown4_alphabet_.size() < kMaxCoarsePosAlphabetSize) break;
    ++brown4_cutoff;
    LOG(INFO) << "Incrementing brown4 cutoff to " << brown4_cutoff << "...";
  }

  while (true) {
    brown6_alphabet_.clear();
    for (int i = 0; i < NUM_SPECIAL_TOKENS; ++i) {
      brown6_alphabet_.Insert(special_symbols[i]);
    }
    for (Alphabet::iterator iter = brown6_alphabet.begin();
         iter != brown6_alphabet.end();
         ++iter) {
      if (brown6_freqs[iter->second] > brown6_cutoff) {
         brown6_alphabet_.Insert(iter->first);
      }
    }
    if (brown6_alphabet_.size() < kMaxCoarsePosAlphabetSize) break;
    ++brown6_cutoff;
    LOG(INFO) << "Incrementing brown6 cutoff to " << brown6_cutoff << "...";
  }
  ...
  pos_alphabet_.StopGrowth();
  cpos_alphabet_.StopGrowth();
  brown4_alphabet_.StopGrowth();
  brown6_alphabet_.StopGrowth();

  LOG(INFO) << "Number of forms: " << form_alphabet_.size() << endl 
		    << "Number of brownall: " << brownall_alphabet_.size() << endl
            ...
            << "Number of brown4: " << brown4_alphabet_.size() << endl
            << "Number of brown6: " << brown6_alphabet_.size();

  CHECK_LT(form_alphabet_.size(), 0xffff);
  CHECK_LT(brownall_alphabet_.size(), 0xffff);
  CHECK_LT(lemma_alphabet_.size(), 0xffff);
  CHECK_LT(prefix_alphabet_.size(), 0xffff);
  CHECK_LT(suffix_alphabet_.size(), 0xffff);
  CHECK_LT(feats_alphabet_.size(), 0xffff);
  CHECK_LT(pos_alphabet_.size(), 0xff);
  CHECK_LT(cpos_alphabet_.size(), 0xff);
  CHECK_LT(brown4_alphabet_.size(), 0xff);
  CHECK_LT(brown6_alphabet_.size(), 0xff);
  // TODO: Remove this (only for debugging purposes).
  BuildNames();
}
